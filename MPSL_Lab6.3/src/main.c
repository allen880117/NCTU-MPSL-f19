...
int main(){
	...
	while(1){
		...
		if(input == -1){
			...
			TIM2->PSC = (uint32_t) 135;
			timerStart();
		} else if ( input == 3){
			//TIM2->PSC = SoundFreq(MI);
			TIM2->PSC = (uint32_t) 120;
			timerStart();
		} else if ( input == 4){
			//TIM2->PSC = SoundFreq(FA);
			TIM2->PSC = (uint32_t) 114;
			timerStart();
		} else if ( input == 5){
			//TIM2->PSC = SoundFreq(SO);
			TIM2->PSC = (uint32_t) 101;
			timerStart();
		} else if ( input == 6){
			//TIM2->PSC = SoundFreq(LA);
			TIM2->PSC = (uint32_t) 90;
			timerStart();
		} else if ( input == 7){
			//TIM2->PSC = SoundFreq(SI);
			TIM2->PSC = (uint32_t) 80;
			timerStart();
		} else if ( input == 8){
			//TIM2->PSC = SoundFreq(HDO);
			TIM2->PSC = (uint32_t) 75;
			timerStart();
		} else {
			timerStop();
		}
